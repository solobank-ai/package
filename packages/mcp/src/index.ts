...
export async function createMcpServer(options: StartMcpServerOptions = {}): Promise<McpServer> {
  ...
  server.tool(
    ...
    'Get the AI Decision Vault state: balance, limits, total decisions, lock status. Read-only.',
    {},
    async () => {
      try {
        if (!(agent as any).getVaultState) return asError(new Error('Vault not available'));
        return asText(await (agent as any).getVaultState());
      } catch (e) { return asError(e); }
    },
  );
  ...
  server.tool(
    ...
    'Deposit funds into the AI Decision Vault.',
    {
      amount: z.number().positive().describe('Amount to deposit'),
    },
    async ({ amount }) => {
      try {
        if (!(agent as any).vaultDeposit) return asError(new Error('Vault not available'));
        const err = checkWrite(amount, guard);
        if (err) return asError(new Error(err));
        const result = await (agent as any).vaultDeposit(amount);
        return asText(result);
      } catch (e) { return asError(e); }
    },
  );
  ...
  server.tool(
    ...
    'AI analyzes DeFi market (Kamino/Marginfi rates) and suggests an action. Uses GPT-4o-mini. Read-only — does not execute.',
    {
      balance: z.number().positive().describe('Available USDC balance to consider'),
    },
    async ({ balance: bal }) => {
      try {
        if (!(agent as any).vaultAnalyze) return asError(new Error('Vault not available'));
        return asText(await (agent as any).vaultAnalyze(bal));
      } catch (e) { return asError(e); }
    },
  );
  ...
  server.tool(
    ...
    'Record an AI decision on-chain. The vault validates limits and creates a permanent on-chain record.',
    {
      decisionType: z.enum(['lend', 'swap', 'rebalance', 'withdraw']).describe('Type of DeFi action'),
      asset: z.string().describe('Asset (USDC, SOL)'),
      amount: z.number().positive().describe('Amount'),
      reasoning: z.string().describe('AI reasoning for this decision'),
    },
    async (args) => {
      try {
        if (!(agent as any).vaultDecide) return asError(new Error('Vault not available'));
        const err = checkWrite(args.amount, guard);
        if (err) return asError(new Error(err));
        const result = await (agent as any).vaultDecide(args);
